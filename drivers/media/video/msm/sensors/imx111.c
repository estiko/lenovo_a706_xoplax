...
static struct msm_camera_i2c_reg_conf imx111_prev_settings[] = {
	...
	{0x4004, 0x02},
#ifdef sicily
	{0x481b, 0x3c},//hs_trail_min
	{0x4837, 0x24},//pclk_period
#endif
};
...
static struct msm_camera_i2c_reg_conf imx111_zsl_settings[] = {
	{0x3035, 0x21},
	{0x3036, 0x2f},
#if defined(CONFIG_HW_ARMANI)|defined(CONFIG_HW_AUDI)
	{0x3821, 0x06},
...
#ifdef sicily
	{0x4800, 0x24},//clock gate enable
	{0x482e, 0x4d},//ui_clk_post_min
#endif
...
static struct msm_camera_i2c_conf_array imx111_confs[] = {
	...
	{&imx111_zsl_settings[0],
	ARRAY_SIZE(imx111_zsl_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
};

static struct msm_camera_csi_params imx111_csi_params = {
...
static struct msm_camera_csi_params *imx111_csi_params_array[] = {
	&imx111_csi_params, /* Snapshot */
	&imx111_csi_params, /* Preview */
	&imx111_csi_params, /* 60fps */
	&imx111_csi_params, /* 90fps */
	&imx111_csi_params, /* ZSL */
};
...
*******************************************************************************/
uint16_t imx111_check_otp(struct msm_sensor_ctrl_t *s_ctrl, uint16_t index)
{
	uint16_t temp, i;
	uint16_t address;
...
*
*******************************************************************************/
void imx111_read_otp(struct msm_sensor_ctrl_t *s_ctrl,
...
*******************************************************************************/
void imx111_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl,
...
*     1, no OTP
*******************************************************************************/
uint16_t imx111_update_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	uint16_t i;
	uint16_t otp_index;
	uint16_t temp;
	...
	for (i = 0; i < 3; i++) {
		temp = imx111_check_otp(s_ctrl, i);
		if (temp == 2) {
			otp_index = i;
			break;
		}
	...
	imx111_read_otp(s_ctrl, otp_index, &st_imx111_otp);
	/* calculate G_gain */
	/* 0x400 = 1x gain */
	if (st_imx111_otp.bg_ratio < OV5647_SUNNY_P5V02S_BG_RATIO_TYPICAL_VALUE) {
		if (st_imx111_otp.rg_ratio < OV5647_SUNNY_P5V02S_RG_RATIO_TYPICAL_VALUE) {
			G_gain = 0x400;
			B_gain = 0x400 *
				OV5647_SUNNY_P5V02S_BG_RATIO_TYPICAL_VALUE /
				st_imx111_otp.bg_ratio;
			R_gain = 0x400 *
				OV5647_SUNNY_P5V02S_RG_RATIO_TYPICAL_VALUE /
				st_imx111_otp.rg_ratio;
		} else {
			R_gain = 0x400;
			G_gain = 0x400 *
				st_imx111_otp.rg_ratio /
	...
	imx111_update_awb_gain(s_ctrl, R_gain, G_gain, B_gain);
	return 0;
}
#endif

static int32_t imx111_write_pict_exp_gain(struct msm_sensor_ctrl_t *s_ctrl,
...
static int32_t imx111_write_prev_exp_gain(struct msm_sensor_ctrl_t *s_ctrl,
						uint16_t gain, uint32_t line)
{
	u8 intg_time_hsb, intg_time_msb, intg_time_lsb;
	uint8_t gain_lsb, gain_hsb;
...
int32_t imx111_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CDBG("imx111_sensor_power_down: %d\n", rc);
	rdata |= 0x18;
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		0x3018, rdata,
		MSM_CAMERA_I2C_BYTE_DATA);
			...
			int update_type, int res)
{
	...
	if (update_type == MSM_SENSOR_REG_INIT) {
		...
		if(!is_first_preview)
		{
			msleep(10);//for 2 frame @15fps, but change to 10 ms in reasonable, chenzc2.
		}
		else
		{
			msleep(10);
...
static struct msm_sensor_fn_t imx111_func_tbl = {
	...
	.sensor_write_exp_gain = imx111_write_prev_exp_gain,
	.sensor_write_snapshot_exp_gain = imx111_write_pict_exp_gain,
	.sensor_csi_setting = imx111_sensor_setting,
...
static struct msm_sensor_ctrl_t imx111_s_ctrl = {
	...
	.csic_params = &imx111_csi_params_array[0],
