...
#if (defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ARMANI) )
extern ssize_t proc_flash_led_write (struct file *file, const char __user *buf, size_t nbytes, loff_t *ppos);
...
extern ssize_t proc_sensor_read (struct file *file, const char __user *buf, size_t nbytes, loff_t *ppos);

const struct file_operations proc_sensor_operations = {
	.owner	= THIS_MODULE,
	.write	= proc_sensor_read,
};
#endif
...
#if (defined(CONFIG_HW_ANDORRA) || defined(CONFIG_HW_ARMANI) || defined(CONFIG_HW_AUDI))
	proc_create_data("CTP_SENSOR_CTRL", S_IFREG | S_IWUGO | S_IWUSR, NULL, &proc_sensor_operations, NULL);
#endif
