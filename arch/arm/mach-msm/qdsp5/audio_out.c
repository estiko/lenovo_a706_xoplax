...
static void audio_dsp_event(void *private, unsigned id, uint16_t *msg)
{
...
#if 0
	static unsigned long tick_first = 0;
	...
	if ( time_after(jiffies, tick_first + 120 * HZ) && time_before(jiffies, tick_first+150*HZ)) {
		printk("----------jiffies:---\n");
		return;
	}
#endif
			   ...
			   size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		rc = wait_event_interruptible_timeout(audio->wait,
					      (frame->used == 0) || (audio->stopped), msecs_to_jiffies(500));
		LOG(EV_WAIT_EVENT, 1);
		if (rc==0) {
