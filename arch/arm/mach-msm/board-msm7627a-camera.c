...
#define QRD_SKUD_GPIO_CAM_3MP_PWDN 117
#define QRD_SKUD_GPIO_CAM_3MP_RESET 130
...
#define GPIO_QRD_ANDORRA_DOVDD 35
#define GPIO_QRD_ANDORRA_AVDD 109
#endif
...
#if !(defined(CONFIG_HW_ANDORRA)||defined(CONFIG_HW_ARMANI)||defined(CONFIG_HW_AUDI) || defined(CONFIG_HW_ATHENAE)||defined(CONFIG_HW_ANDORRAP))
...
static struct msm_camera_sensor_flash_src msm_flash_src_ov5647 = {
	...
	._fsrc.ext_driver_src.led_en = 23,
...
static void __init msm7x27a_init_cam(void)
{
	...
	{  //for SKUD
		...
		msm_camera_sensor_ov5648_truly_cm8352_data.sensor_reset=QRD_SKUA_GPIO_CAM_5MP_RESET;
		msm_camera_sensor_ov5648_truly_cm8352_data.sensor_pwd = QRD_SKUA_GPIO_CAM_5MP_SHDN_EN;
		...
		//sensor_board_info_ov8825.cam_vreg = ov8825_gpio_vreg;
		//sensor_board_info_ov8825.num_vreg = ARRAY_SIZE(ov8825_gpio_vreg);
		//msm_act_main_cam_3_info.vcm_pwd = 0;
		//msm_act_main_cam_3_info.vcm_enable = 0;
		msm_camera_sensor_ov8825_data.sensor_reset = QRD_SKUA_GPIO_CAM_5MP_RESET;
		msm_camera_sensor_ov8825_data.sensor_pwd = QRD_SKUA_GPIO_CAM_5MP_SHDN_EN;
		sensor_board_info_ov8825.mount_angle = 90;
#if defined(CONFIG_HW_ANDORRA_LDO)
		pr_err("avdd dovdd setting OK!!!\n");
		msm_camera_sensor_ov8825_data.sensor_dovdd = GPIO_QRD_ANDORRA_DOVDD;
		msm_camera_sensor_ov8825_data.sensor_avdd = GPIO_QRD_ANDORRA_AVDD;
#endif
#endif

#ifdef CONFIG_MT9V113
		msm_camera_sensor_mt9v113_data.sensor_reset = QRD_SKUD_GPIO_CAM_3MP_RESET;
		msm_camera_sensor_mt9v113_data.sensor_pwd = QRD_SKUD_GPIO_CAM_3MP_PWDN;
		msm_camera_sensor_mt9v113_data.sensor_reset_enable = 1;
#endif

#ifdef CONFIG_SP0A28
		msm_camera_sensor_sp0a28_data.sensor_reset = QRD_SKUD_GPIO_CAM_3MP_RESET;
		msm_camera_sensor_sp0a28_data.sensor_pwd = QRD_SKUD_GPIO_CAM_3MP_PWDN;
		msm_camera_sensor_sp0a28_data.sensor_reset_enable = 1;
#endif
#ifdef CONFIG_OV7695
...
static void skud_camera_gpio_cfg(void)
{
    ...
    pr_err("%s IN\r\n", __func__);
#if defined(CONFIG_HW_ANDORRA_LDO)
	rc = gpio_request(GPIO_QRD_ANDORRA_DOVDD, "ov8825");
	if (rc < 0)
		pr_err("%s: gpio_request---GPIO_QRD_ANDORRA_AVDD failed!",
				__func__);
	rc = gpio_request(GPIO_QRD_ANDORRA_AVDD, "ov8825");
	if (rc < 0)
		pr_err("%s: gpio_request---GPIO_QRD_ANDORRA_AVDD failed!",
				__func__);
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_QRD_ANDORRA_DOVDD, 0,
				GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("%s: unable to enable Dovdd for main"
				"camera!\n", __func__);
		gpio_free(GPIO_QRD_ANDORRA_DOVDD);
	}
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_QRD_ANDORRA_AVDD, 0,
				GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("%s: unable to enable Avdd for main"
				"camera!\n", __func__);
		gpio_free(GPIO_QRD_ANDORRA_AVDD);
	}
#endif
#if defined(CONFIG_HW_ANDORRA)
	rc = gpio_request(QRD_SKUA_GPIO_CAM_5MP_SHDN_EN, "ov8825");
	if (rc < 0)
		pr_err("%s: gpio_request---GPIO_CAM_5MP_SHDN_EN failed!",
				__func__);
#else
	...
	rc = gpio_request(QRD_SKUA_GPIO_CAM_5MP_RESET, "ov8825");
	if (rc < 0) {
		pr_err("%s: gpio_request---GPIO_CAM_5MP_RESET failed!",
				__func__);
		gpio_free(QRD_SKUA_GPIO_CAM_5MP_SHDN_EN);
	...
	rc = gpio_request(QRD_SKUD_GPIO_CAM_3MP_PWDN, "mt9v113");
	if (rc < 0)
		pr_err("%s: gpio_request---GPIO_CAM_3MP_PWDN failed!",
				__func__);

	rc = gpio_tlmm_config(GPIO_CFG(QRD_SKUD_GPIO_CAM_3MP_PWDN, 0,
				GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("%s: unable to enable Power Down gpio for front"
				"camera!\n", __func__);
		gpio_free(QRD_SKUD_GPIO_CAM_3MP_PWDN);
	}

	rc = gpio_request(QRD_SKUD_GPIO_CAM_3MP_RESET, "mt9v113");
	if (rc < 0) {
		pr_err("%s: gpio_request---GPIO_CAM_5MP_RESET failed!",
				__func__);
		gpio_free(QRD_SKUD_GPIO_CAM_3MP_PWDN);
	...
	rc = gpio_tlmm_config(GPIO_CFG(QRD_SKUD_GPIO_CAM_3MP_RESET, 0,
				GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("%s: unable to enable reset gpio for main camera!\n",
				__func__);
		gpio_free(QRD_SKUD_GPIO_CAM_3MP_RESET);
    }
///enable
#if defined(CONFIG_HW_ANDORRA_LDO)
	gpio_direction_output(GPIO_QRD_ANDORRA_DOVDD, 1);
	gpio_direction_output(GPIO_QRD_ANDORRA_AVDD, 1);
	msleep(10);
#endif
	gpio_direction_output(QRD_SKUA_GPIO_CAM_5MP_SHDN_EN, 1);
	gpio_direction_output(QRD_SKUA_GPIO_CAM_5MP_RESET, 1);

	gpio_direction_output(QRD_SKUD_GPIO_CAM_3MP_PWDN, 1);
	gpio_direction_output(QRD_SKUD_GPIO_CAM_3MP_RESET, 1);
}
/*{
	int rc = 0;
...
void __init msm7627a_camera_init(void)
{
	...
	if (machine_is_msm7627a_qrd1()) {
		...
		if (machine_is_msm7627a_evb()|| machine_is_msm8625_evb()){
			pr_err("i2c_camera_devices_evb registered\n");
			i2c_register_board_info(MSM_GSBI0_QUP_I2C_BUS_ID,
					i2c_camera_devices,
					ARRAY_SIZE(i2c_camera_devices));
		}else if(machine_is_msm8625_qrd5()|| machine_is_msm7x27a_qrd5a()){
			pr_err("i2c_camera_devices_sku5 registered\n");
			i2c_register_board_info(MSM_GSBI0_QUP_I2C_BUS_ID,
					i2c_camera_devices_sku5,
					ARRAY_SIZE(i2c_camera_devices_sku5));
		}else if(machine_is_msm7627a_qrd3()|| machine_is_msm8625_qrd7()){
			pr_err("i2c_camera_devices_sku7 registered\n");
			i2c_register_board_info(MSM_GSBI0_QUP_I2C_BUS_ID,
					i2c_camera_devices_sku7,
					ARRAY_SIZE(i2c_camera_devices_sku7));
		}else if(machine_is_msm8625q_skud())
		{
			pr_err("i2c_camera_devices_SKUD registered\n");
			i2c_register_board_info(MSM_GSBI0_QUP_I2C_BUS_ID,
